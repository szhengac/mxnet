...
__global__ void KernelStep1(const MultiNesLAMBKernelParam<DType, MPDType> kernel_params,
...
template<bool has_mixed_precision, typename MPDType, typename DType>
__global__ void KernelStep2(const MultiNesLAMBKernelParam<DType, MPDType> kernel_params,
                            const float beta1,
                            const MPDType beta3,
...
.set_attr<FCompute>("FCompute<gpu>",  MultiNesLAMBUpdate<gpu, false>);

NNVM_REGISTER_OP(_multi_mp_neslamb_update)
.set_attr<FCompute>("FCompute<gpu>",  MultiNesLAMBUpdate<gpu, true>);
