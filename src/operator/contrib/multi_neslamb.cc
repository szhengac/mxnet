...
struct MultiNesLAMBKernelStep1 {
...
template<typename MPDType, bool has_mixed_precision>
struct MultiNesLAMBKernelStep2 {
  template<typename DType>
  MSHADOW_XINLINE static void Map(int i,
                                  const MultiNesLAMBKernelParam<DType, MPDType>& kernel_params,
                                  const float beta1,
...
void CallKernel1(Stream<cpu>* s,
                  ...
                  int* block_to_tensor,
                  int* block_to_chunk) {
  Kernel<MultiNesLAMBKernelStep1<MPDType, !std::is_same<DType, MPDType>::value>, cpu>::
...
void CallKernel2(Stream<cpu>* s,
                  ...
                  int* block_to_chunk,
                  const OpReqType req) {
  Kernel<MultiNesLAMBKernelStep2<MPDType, !std::is_same<DType, MPDType>::value>, cpu>::
                                  Launch(s, kernel_params.max_size,
                                  kernel_params,
                                  param.beta1,
                                  r1, r2_m, r2_g,
...
DMLC_REGISTER_PARAMETER(MultiNesLAMBParam);

std::vector<std::string> NesLAMBParamToVector(uint32_t num_tensors,
                                              const char *p_names[],
                                              size_t n_params) {
...
inline uint32_t NumTensors(const nnvm::NodeAttrs& attrs) {
  return static_cast<uint32_t>(dmlc::get<MultiNesLAMBParam>(attrs.parsed).num_tensors);
}

NNVM_REGISTER_OP(_multi_neslamb_update)
.describe(R"code(Compute the Nesterov LAMB coefficients of multiple weights and grads"
)code" ADD_FILELINE)
...
.set_attr_parser(ParamParser<MultiNesLAMBParam>)
.set_attr<mxnet::FInferShape>("FInferShape", MultiNesLAMBInferShape<MultiNesLAMBParam, 4>)
.set_attr<nnvm::FInferType>("FInferType", ElemwiseType<-1, -1>)
.set_attr<nnvm::FListInputNames>("FListInputNames",
  [](const NodeAttrs& attrs) {
    const char *param_names[] = {"weight_", "grad_", "mean_", "var_"};
    return NesLAMBParamToVector(NumTensors(attrs), param_names,
...
.set_attr<FCompute>("FCompute<cpu>", MultiNesLAMBUpdate<cpu, false>)
.add_argument("data", "NDArray-or-Symbol[]", "data")
.add_arguments(MultiNesLAMBParam::__FIELDS__());


NNVM_REGISTER_OP(_multi_mp_neslamb_update)
...
.set_attr_parser(ParamParser<MultiNesLAMBParam>)
.set_attr<mxnet::FInferShape>("FInferShape", MultiNesLAMBInferShape<MultiNesLAMBParam, 5>)
.set_attr<nnvm::FInferType>("FInferType", MPMultiNesLAMBInferType<MultiNesLAMBParam, 5>)
.set_attr<nnvm::FListInputNames>("FListInputNames",
  [](const NodeAttrs& attrs) {
...
.set_attr<FCompute>("FCompute<cpu>", MultiNesLAMBUpdate<cpu, true>)
.add_argument("data", "NDArray-or-Symbol[]", "data")
.add_arguments(MultiNesLAMBParam::__FIELDS__());
