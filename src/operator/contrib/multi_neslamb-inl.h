 ...
 * \file multi_neslamb-inl.h
 * \brief multi-tensor Nesterov LAMB optimizer with normalized gradient
 * \author Shuai Zheng
 */
#ifndef MXNET_OPERATOR_CONTRIB_MULTI_NESLAMB_INL_H_
#define MXNET_OPERATOR_CONTRIB_MULTI_NESLAMB_INL_H_
...
namespace multineslamb {
enum MultiNesLambUpdateResource {kTempSpace};
}  // namespace multineslamb

struct MultiNesLAMBParam : public dmlc::Parameter<MultiNesLAMBParam> {
  ...
  DMLC_DECLARE_PARAMETER(MultiNesLAMBParam) {
...
inline bool MultiNesLAMBInferShape(const nnvm::NodeAttrs& attrs,
...
template <typename ParamType, int input_stride>
inline bool MPMultiNesLAMBInferType(const nnvm::NodeAttrs& attrs,
...
template<typename T>
class NesLAMBTypeIdentity {
 public:
  using type = T;
};

template<typename T>
class NesLAMBSinglePrecision {
 public:
  using type = float;
};

template<typename DType, typename MPDType>
struct MultiNesLAMBKernelParam {
...
template<typename xpu,
         typename DType,
         typename MPDType,
         typename ParamType = MultiNesLAMBParam,
         int input_stride = 5>
void FillMultiNesLAMBKernelParam(const nnvm::NodeAttrs& attrs,
                                 const OpContext &ctx,
                                 const std::vector<TBlob> &inputs,
                                 const std::vector<TBlob> &outputs,
                                 MultiNesLAMBKernelParam<DType, MPDType> *multi_param) {
...
inline void MultiNesLAMB(const nnvm::NodeAttrs& attrs,
                         const OpContext &ctx,
                         const std::vector<TBlob> &inputs,
                         const std::vector<OpReqType> &req,
                         const std::vector<TBlob> &outputs) {
  auto param = nnvm::get<MultiNesLAMBParam>(attrs.parsed);
  Stream<xpu>* s = ctx.get_stream<xpu>();

  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    using MPDType = typename MPTypeChooser<DType>::type;
    MultiNesLAMBKernelParam<DType, MPDType> kernel_params;
    FillMultiNesLAMBKernelParam<xpu, DType, MPDType, MultiNesLAMBParam, input_stride>
    ...
    ctx.requested[multineslamb::kTempSpace].get_space_typed<xpu, 1, char>(
...
inline void MultiNesLAMBUpdate(const nnvm::NodeAttrs& attrs,
                               const OpContext &ctx,
                               const std::vector<TBlob> &inputs,
                               const std::vector<OpReqType> &req,
                               const std::vector<TBlob> &outputs) {
  if (!MP) {
    MultiNesLAMB<xpu, NesLAMBTypeIdentity, 4>
      (attrs, ctx, inputs, req, outputs);
  } else {
    MultiNesLAMB<xpu, NesLAMBSinglePrecision, 5>
      (attrs, ctx, inputs, req, outputs);
  }
}
...
#endif  // MXNET_OPERATOR_CONTRIB_MULTI_NESLAMB_INL_H_
