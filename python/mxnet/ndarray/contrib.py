...
def multi_neslamb_update(weights, grads, mean, var, step_count,
                         lrs, wds, out=None, num_tensors=0, **kwargs):
    """Given a list of gradients, update weights, mean and variance of multiple tensors
    following Nesterov LAMB Optimizer implementation.
    ...
    if not num_tensors:
        num_tensors = len(weights)
    temp_list = _flatten_list(zip(weights, grads, mean, var))
    return ndarray._internal._multi_neslamb_update(*temp_list,
                                                   out=out,
                                                   num_tensors=num_tensors,
                                                   step_count=step_count,
                                                   learning_rates=lrs,
...
def multi_mp_neslamb_update(weights, grads, mean, var, weights32, step_count,
                            lrs, wds, out=None, num_tensors=0, **kwargs):
    """Given a list of gradients, update weights, mean and variance of multiple tensors
    following Nesterov LAMB Optimizer implementation, and using Mixed-Precision.
    ...
    if not num_tensors:
        num_tensors = len(weights)
    temp_list = _flatten_list(zip(weights, grads, mean, var, weights32))
    return ndarray._internal._multi_mp_neslamb_update(*temp_list,
                                                      out=out,
                                                      num_tensors=num_tensors,
                                                      step_count=step_count,
                                                      learning_rates=lrs,
